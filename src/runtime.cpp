...
*/
    ...
    */
    ...
    // ==========================================
    // ACTIONOBJECTS GL,UI,NET,COMPONENTS
    // ==========================================
    nativeRegistry["actionobjectsui_get_char"] = [](const std::vector<Obj>& args) -> Obj {
        return Obj(SysGui::getCharPressed());
    };
    nativeRegistry["actionobjectsui_get_key"] = [](const std::vector<Obj>& args) -> Obj {
        return Obj(SysGui::getKeyPressed());
    };
    nativeRegistry["actionobjectsui_is_key_down"] = [this](const std::vector<Obj>& args) -> Obj {
        if (args.empty()) return Obj(false);
        return Obj(SysGui::isKeyDown(objToString(args[0])));
    };

    nativeRegistry["actionobjectsui_measure_text"] = [this](const std::vector<Obj>& args) -> Obj {
        if (args.size() < 2) return Obj(0);
    ...
    nativeRegistry["actionobjectsui_setup"] = [this](const std::vector<Obj>& args) -> Obj {
    ...
    nativeRegistry["actionobjectsui_close"] = [](const std::vector<Obj>& args) -> Obj {
        SysGui::close();
        return Obj(0);
    };

    nativeRegistry["actionobjectsui_running"] = [](const std::vector<Obj>& args) -> Obj {
        return Obj(SysGui::running());
    };

    nativeRegistry["actionobjectsui_start"] = [](const std::vector<Obj>& args) -> Obj {
        SysGui::start();
        return Obj(0);
    };
    
    nativeRegistry["actionobjectsui_present"] = [](const std::vector<Obj>& args) -> Obj {
        SysGui::present();
        return Obj(0);
    };

    nativeRegistry["actionobjectsui_clear"] = [this](const std::vector<Obj>& args) -> Obj {
    ...
    nativeRegistry["actionobjectsui_text"] = [this](const std::vector<Obj>& args) -> Obj {
    if (args.size() < 3) return Obj(0);
    ...
    nativeRegistry["actionobjectsui_rect"] = [this](const std::vector<Obj>& args) -> Obj {
	...
	nativeRegistry["actionobjectsui_is_mouse_down"] = [this](const std::vector<Obj>& args) -> Obj {
        return Obj(SysGui::isMouseDown());
    };

    nativeRegistry["actionobjectsui_mouse_x"] = [](const std::vector<Obj>& args) -> Obj {
    ...
    nativeRegistry["actionobjectsui_click"] = [](const std::vector<Obj>& args) -> Obj {
        return Obj(SysGui::isMousePressed());
    };
    
    nativeRegistry["actionobjectsui_key"] = [this](const std::vector<Obj>& args) -> Obj { 
        if (args.empty()) return Obj(false);
        std::string key = objToString(args[0]);
        return Obj(SysGui::isKeyDown(key));
    };
    nativeRegistry["actionobjectsui_load_font"] = [this](const std::vector<Obj>& args) -> Obj {
        if (args.size() < 2) return Obj(0);
        std::string path = objToString(args[0]);
        int size = std::stoi(objToString(args[1]));
    ...
    nativeRegistry["actionobjectsui_is_key_pressed"] = [this](const std::vector<Obj>& args) -> Obj {
        if (args.empty()) return Obj(false);
        std::string key = objToString(args[0]);
    ...
    nativeRegistry["actionobjectsui_load_image"] = [this](const std::vector<Obj>& args) -> Obj {
        if (args.size() < 2) return Obj(false);
        std::string path = objToString(args[0]);
    ...
    nativeRegistry["actionobjectsui_draw_image"] = [this](const std::vector<Obj>& args) -> Obj {
    ...
    nativeRegistry["actionobjectsui_quit"] = [](const std::vector<Obj>& args) -> Obj {
    SysGui::stop();
    return Obj(true);
    };
